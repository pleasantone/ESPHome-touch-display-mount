...
**Sync base: upstream `d8f84f7`** (#33, merged 2026-09-24). Next time upstream moves, diff `d8f84f7..origin/main -- esphome/home-like/cyd-2432s028/home-like.yaml` and port the result into the two personal configs by hand — they have diverged too far (flex grid, 8 tiles, availability, board model) for the patch to apply. On `local-panels`, merge `origin/main` and take upstream's side of every shared file; the only files that differ from upstream there are the personal configs, the 320×480 image, `.gitignore` and this file.
...
- ILI9342 is four extra presets in `cyd-2432s028`, using the generic **`ILI9342` chip model** + `RGB` + 40MHz — not the board model. Our panel keeps the board model. Upstream's chip-model 180° preset was flashed on our panel on 2026-09-24: upright, colours right, touch aligned, and **no visible contrast difference** from the board model. The result was reported on #33.
